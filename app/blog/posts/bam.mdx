...
We compare AdamW, Muon, and BAM on three representative settings: a CNN (ResNet-18 on CIFAR-10), an MLP (a 4-layer ReLU MLP on CIFAR-10), and a small transformer (nanoGPT on FineWeb). In all cases we fix the dataset, architecture, and training budget. To isolate the effect of the matrix-update transform, we use the same parameter split in every run: non-2D parameters are always optimized with AdamW using fixed hyperparameters, while hidden 2D weight matrices use AdamW, Muon, or BAM.

For each experiment we sweep learning rate $\eta$ and batch size $B$ (effective token-batch size for nanoGPT). Each plotted point reports the best final metric at fixed $\eta$ (left) or fixed $B$ (right), maximizing over the remaining sweep dimensions. Full sweep grids and parameter-group definitions are provided in the Appendix.
...
<div class="flex flex-col gap-4">
  ...
  <img src="/bam/cnn_perf.png" alt="ResNet-18 CIFAR-10 sweep results" width="80%" />
      <figcaption><em>ResNet-18 on CIFAR-10 (100 epochs, top-1 test accuracy)</em> </figcaption>
  </figure>
  <figure>
    <img src="/bam/mlp_perf.png" alt="4-layer MLP CIFAR-10 sweep results" width="80%" />
      <figcaption><em>4-layer MLP on CIFAR-10 (50 epochs, top-1 test accuracy)</em> </figcaption>
  </figure>
  <figure>
    <img src="/bam/nanogpt_perf.png" alt="nanoGPT FineWeb sweep results" width="80%" />
      <figcaption><em>nanoGPT on FineWeb (700M tokens, final validation loss)</em> </figcaption>
  </figure>
</div>
</center>
...
[^1]: Loshchilov, I. and Hutter, F. (2019) 'Decoupled Weight Decay Regularization'. Available at: http://arxiv.org/abs/1711.05101.
[^2]: Gupta, V., Koren, T. and Singer, Y. (2018) 'Shampoo: Preconditioned Stochastic Tensor Optimization'. Available at: http://arxiv.org/abs/1802.09568.
[^3]: Vyas, N. et al. (2025) 'SOAP: Improving and Stabilizing Shampoo using Adam'. Available at: http://arxiv.org/abs/2409.11321.
[^4]: Jordan, K. et al. (2024) 'Muon: An optimizer for hidden layers in neural networks'. Available at: https://kellerjordan.github.io/posts/muon/.
