...
> This is a crosspost from the [Tensara blog](https://tensara.org/blog/how-wrong-is-too-wrong).

For context, I help run [Tensara](https://tensara.org/), a competitive GPU programming platform with [Somesh](https://someshkar.com/), [Soham](https://sohamjog.com/), and [Harmya](https://harmya.me/). If you haven't heard of it before, the gist is to compete to write the fastest GPU kernel for a given mathematical workload (GEMM, Conv, Attention, etc).
...
Even among wrong submissions, the errors are often tiny: maximum differences frequently fall in the range $[10^{-5}, 10^{-3}]$, while mean differences reach down to $10^{-9}$, with many below $10^{-4}$. 
